...
namespace HCP.Customer.WebAPI.Controllers
{
    ...
    public class CustomerMasterController : ControllerBase
    {
        ...
        /// UI Reffered - Customer
        /// </summary>
        /// <param name="customerId"></param>
        /// <returns></returns>
        [HttpGet]
        public async Task<IActionResult> ActiveOrDeActiveCustomer(bool status, int customerId)
        {
            var msg = await _CustomerMasterRepository.ActiveOrDeActiveCustomer(status, customerId);
            return Ok(msg);
        }
